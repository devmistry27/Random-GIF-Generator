...
const Tag = () => {
    ...
    return (
        <div className='flex flex-col gap-4 py-4 justify-center items-center w-full lg:w-1/2 overflow-hidden aspect-auto bg-blue-400 border-2 border-gray-500 rounded-2xl'>
            ...
            {
                loading ? (<Spinner />) : (<img src={gif} alt='RandomGIF' className='max-w-[30rem] px-4' />)
            }
            ...
            <div className='w-full flex flex-col items-center'>
                <input
                    ...
                    value={tag}
                />
                <button onClick={() => fetchData(tag)} className='uppercase w-4/5 py-2 bg-white/70 rounded-lg my-2 hover:bg-white transition-all font-semibold'>
                    Generate</button>
            </div>
